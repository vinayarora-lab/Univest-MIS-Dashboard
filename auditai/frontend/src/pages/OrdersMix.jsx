...
export default function OrdersMix() {
  const [data, setData] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    api.get('/api/datapack/orders-mix')
  ...
  const chartData = last12Dates.map((d, i) => {
    ...
    const entry = { date: d };
    nonTotalPlans.forEach(p => { entry[p.plan] = p.values[idx] || 0; });
    return entry;
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
        <h2 className="font-semibold text-gray-800 text-sm mb-4">Orders by Plan (Stacked) — Last 12 Months</h2>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-200">
          <h2 className="font-semibold text-gray-800 text-sm">Orders Mix by Plan — Last 12 Months</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {plans.map((plan, i) => (
                <tr key={i} className={`border-b border-gray-100 ${plan.isTotal ? 'bg-blue-50 font-semibold' : 'hover:bg-gray-50'}`}>
